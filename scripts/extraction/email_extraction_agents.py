...
async def process_email_with_agents(msg_path: Path) -> Dict:
    """
    ...
    try:
        ...
        enhancement = await enhance_email_with_agent(msg_path, base_markdown)
...
def process_email_with_agents_sync(msg_path: Path) -> Dict:
    """Synchronous wrapper for async email processing."""
    return asyncio.run(process_email_with_agents(msg_path))
